...
const { User, Restaurant, Meal, Order } = require('../server/db/models')
...
async function seed () {
  ...
  const meals = await Promise.all([
    ...
    Meal.create({ name: 'Steak Burrito', inStorePrice: 1000, basePrice: 650, image: 'http://img.thedailybeast.com/image/upload/v1492725220/articles/2013/07/31/chipotle-and-the-terrible-horrible-no-good-very-bad-vegan-burrito/130730-Ross-Chipotle-tease_qe5dgb.jpg', restaurantId: restaurants[2].id }),
  ...
  console.log(chalk.bgRed.white.bold(`seeded ${users.length} users, ${restaurants.length} restaurants, ${meals.length} meals, ${orders.length} orders`))
